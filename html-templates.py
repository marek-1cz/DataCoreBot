...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
    <div style="background: var(--bg-panel); padding: 20px; border-radius: 10px;">
        ...
        <div style="position: relative; height: 250px; width: 100%;"><canvas id="chart7d"></canvas></div>
    </div>
    <div style="background: var(--bg-panel); padding: 20px; border-radius: 10px;">
        <h3 style="color: var(--blue-main); margin-top: 0;"><i class="fas fa-clock"></i> Dnešní aktivita po hodinách</h3>
        <div style="position: relative; height: 250px; width: 100%;"><canvas id="chart24h"></canvas></div>
    </div>
</div>
...
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    ...
    <table style="width: 100%;">
        <tr><th>Stát / Region</th><th>Počet zobrazení</th></tr>
        ...
        <tr>
            <td style="font-weight: bold; color: var(--text-main); display: flex; align-items: center; gap: 10px;">
                {% if data.flag %}<img src="{{ data.flag }}" alt="" style="border-radius: 3px; box-shadow: 0 0 5px rgba(0,0,0,0.5);">{% endif %}
                {{ c_name }}
            </td>
...
<div style="display: flex; gap: 20px; flex-wrap: wrap;">
    <div style="flex: 1; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px; border-top: 4px solid {{ 'var(--success)' if soft_enabled else 'var(--danger)' }}; text-align: center;">
        ...
        <div style="font-size: 50px; margin: 15px 0; color: {{ 'var(--success)' if soft_enabled else 'var(--danger)' }}; text-shadow: 0 0 15px {{ 'rgba(16, 185, 129, 0.5)' if soft_enabled else 'rgba(239, 68, 68, 0.5)' }};"><i class="fas {{ 'fa-check-circle' if soft_enabled else 'fa-ban' }}"></i></div>
    ...
    <div style="flex: 1; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px; border-top: 4px solid {{ 'var(--success)' if dl_enabled else 'var(--danger)' }}; text-align: center;">
        ...
        <div style="font-size: 50px; margin: 15px 0; color: {{ 'var(--success)' if dl_enabled else 'var(--danger)' }}; text-shadow: 0 0 15px {{ 'rgba(16, 185, 129, 0.5)' if dl_enabled else 'rgba(239, 68, 68, 0.5)' }};"><i class="fas {{ 'fa-check-circle' if dl_enabled else 'fa-ban' }}"></i></div>
...
<div style="text-align: center; padding: 60px 20px; max-width: 700px; margin: 50px auto; background-color: var(--bg-panel); border-radius: 15px; box-shadow: 0 15px 30px rgba(0,0,0,0.5); border-top: 5px solid #5865F2;">
    ...
    <div style="background-color: rgba(88, 101, 242, 0.1); border: 1px solid #5865F2; padding: 30px 20px; border-radius: 10px; margin: 30px 20px;">
        ...
        <p style="color: var(--text-muted); font-size: 14px; margin-bottom: 30px;">Připojte se na náš Discord, ověřte, že nejste robot, a poté přejděte do kanálu <b>💾・download</b>, kde stačí postupovat podle pokynů DataCoreBota. 🚀</p>
...
<div style="display: flex; justify-content: center; flex-wrap: wrap; gap: 20px;">
    {% for member in team %}
    <div style="background-color: var(--bg-panel); border-radius: 10px; padding: 20px; text-align: center; border-top: 4px solid var(--blue-main); width: 300px; max-width:100%; transition: transform 0.5s ease, box-shadow 0.5s ease;" onmouseover="this.style.transform='translateY(-8px)';this.style.boxShadow='0 12px 30px rgba(56,189,248,0.4)';" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none';">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        {% for r in role_list %}
                            {% set r_clean = r.strip() %}
                            {% if r_clean == 'SA' %}<span class="role-tag" style="color: white; background-color: #ef4444; border-color: #ef4444;">SERVER ADMIN</span>
                            {% elif r_clean == 'DEV' %}<span class="role-tag" style="color: white; background-color: #10b981; border-color: #10b981;">DEVELOPER</span>
                            {% elif r_clean == 'BT' %}<span class="role-tag" style="color: white; background-color: #3b82f6; border-color: #3b82f6;">BETA TESTER</span>
                            {% elif r_clean == 'User' %}<span class="role-tag" style="color: white; background-color: #64748b; border-color: #64748b;">User</span>{% endif %}
                        {% endfor %}
                    </td>
                    <td>
    ...
    <div style="overflow-x: auto;">
        <table>
            ...
            <tr style="opacity: {{ '0.6' if user.get('is_deleted') else '1' }};">
                ...
                <td>
                    {% if user.get('is_deleted') %}<span style="color: var(--danger); font-size: 12px; font-weight: bold;">Smazán (Blokuje ID)</span>
                    {% else %}<span style="color: var(--success); font-size: 12px;">Aktivní</span>{% endif %}
                </td>
                <td>
    ...
    <div style="overflow-x: auto;">
        <table id="usersTable">
            ...
            <tr>
                ...
                <td><strong>{{ user.get('nick', '') }}</strong></td>
                <td>
                    {% if user.get('is_banned') %}<span style="color: var(--danger); font-size: 11px; font-weight:bold; border:1px solid var(--danger); padding:2px 5px; border-radius:4px;">BANNED</span>
                    {% elif user.get('is_deleted') %}<span style="color: var(--text-muted); font-size: 11px; font-weight:bold; border:1px solid var(--text-muted); padding:2px 5px; border-radius:4px;">DELETED</span>
                    {% elif not user.get('hwid') or user.get('hwid') == 'None' or user.get('hwid') == '' %}<span style="color: var(--warning); font-size: 11px; font-weight:bold; border:1px solid var(--warning); padding:2px 5px; border-radius:4px;">NOT ACTIVATED</span>
                    {% else %}<span style="color: var(--success); font-size: 11px; font-weight:bold; border:1px solid var(--success); padding:2px 5px; border-radius:4px;">ACTIVATED</span>{% endif %}
                </td>
                ...
                <td data-sort="{{ role_weight }}">
                    ...
                    {% for r in role_list %}
                        {% set r_clean = r.strip() %}
                        {% if r_clean == 'SA' %}<span class="role-tag" style="background-color: #ef4444; color: white;">SA</span>
                        {% elif r_clean == 'DEV' %}<span class="role-tag" style="background-color: #10b981; color: white;">DEV</span>
                        {% elif r_clean == 'BT' %}<span class="role-tag" style="background-color: #3b82f6; color: white;">BT</span>
                        {% elif r_clean == 'User' %}<span class="role-tag" style="background-color: #64748b; color: white;">User</span>{% endif %}
                    {% endfor %}
                ...
                <td style="color: var(--text-muted); font-size: 13px;" data-sort="{{ '99999999999' if user.get('is_online') else user.get('last_active', '0') }}">
                    {% if user.get('is_online') %}<span style="color: var(--success); font-weight: bold;">🟢 AKTIVNÍ</span>
                    {% else %}{{ user.get('last_active', 'Nikdy nehrál') }}{% endif %}
                </td>
                <td>
    ...
    function sortTable(n) {
        ...
        while (switching) {
            ...
            for (i = 1; i < (rows.length - 1); i++) {
                ...
                if (!isNaN(xContent) && !isNaN(yContent)) { xContent = parseFloat(xContent); yContent = parseFloat(yContent); } 
                else { xContent = xContent.toLowerCase(); yContent = yContent.toLowerCase(); }
...
<div style="max-width: 800px; margin: 0 auto; padding: 20px; position: relative;">
    <div style="text-align: center; margin-bottom: 40px;">
        ...
        <a href="https://www.buymeacoffee.com/marekk_czz" target="_blank" class="glowing-btn-blue"><i class="fas fa-heart"></i> Podpořit Projekt</a>
    </div>
            ...
            <tr>
                ...
                <td>
                    <form action="/dashboard/approve_claim" method="POST" style="display:inline;">
                        ...
                        <button type="submit" class="btn btn-success" style="padding: 5px 10px; font-size: 12px;"><i class="fas fa-check"></i> Schválit</button>
                    </form>
                    <form action="/dashboard/reject_claim" method="POST" style="display:inline;">
                        <input type="hidden" name="claim_id" value="{{ p.get('id', '') }}">
                        <button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;" onclick="return confirm('Opravdu zamítnout?')"><i class="fas fa-times"></i></button>
                    </form>
                </td>
            </tr>
...
<div style="display: flex; gap: 20px; flex-wrap: wrap;">
    ...
    <div style="flex: 2; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
        ...
        <div style="overflow-x: auto;">
            <table>
                <tr><th>Jméno</th><th>Discord</th><th>Částka</th><th>Zpráva</th><th>Datum</th><th>Akce</th></tr>
                {% for s in supporters %}
                <tr>
                    ...
                    <td>
                        <form action="/dashboard/delete_supporter" method="POST" style="display:inline;">
                            ...
                            <button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;" onclick="return confirm('Opravdu smazat?')"><i class="fas fa-trash"></i></button>
                        </form>
                    </td>
                </tr>
                {% else %}<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Zatím žádné platby.</td></tr>{% endfor %}
            </table>
        </div>
    </div>
</div>
