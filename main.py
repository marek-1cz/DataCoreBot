...
def check_version_access(db, app_version_from_pc, user):
    ...
    if not app_version_from_pc or str(app_version_from_pc).strip() == "": return {"allowed": False, "msg": "Nepodporovaná verze."}
    try:
        v_data = db.table("software_versions").select("*").eq("db_version", app_version_from_pc).execute().data
        if not v_data: return {"allowed": False, "msg": "Verze neexistuje!"}
        v_info = v_data[0]
        if str(v_info.get("is_active", "True")).lower() == "false": return {"allowed": False, "msg": "Nepodporovaná verze."}
        ...
        if target != "User":
            ...
            if "SA" not in roles and "DEV" not in roles:
                if target == "BT" and "BT" not in roles: return {"allowed": False, "msg": "Omezeno pro BT."}
                elif target == "DEV_SA": return {"allowed": False, "msg": "Neveřejná verze."}
        return {"allowed": True}
    except Exception: return {"allowed": True}
...
def public_stats():
    db = get_db()
    if not db: return redirect(url_for('home'))
    ...
    return render_public(HTML_PUBLIC_STATS, user_ver=user_ver, bt_ver=bt_ver, activated_users=activated_users, total_supporters=total_supporters, total_hours=total_hours, total_launches=total_launches, top_time=top_time, top_launches=top_launches, searched_user=searched_user)
...
def claim_role():
    if request.method == 'POST':
        ...
        if valid_records:
            ...
            if expired_records: flash('Časový limit 24 hodin vypršel.', 'warning')
                ...
                send_log("🚨 VYŽADUJE KONTROLU", f"Uživatel **{discord_nick}** žádá o platbu od **{bmac_name}**, ale webhookem neprošla.", 0xef4444)
                flash('Platba nenalezena. Odesláno k ruční kontrole.', 'warning')
        return redirect(url_for('claim_role'))
...
def api_app_login():
    ...
    try:
        ...
        if user.get("is_banned"): return _cors_jsonify({"status": "banned", "message": "Tento účet má BAN."})
        ...
        if db_hwid and str(db_hwid) != "None" and str(db_hwid).strip() != "":
            if str(db_hwid) != req_hwid: return _cors_jsonify({"status": "hwid_error", "message": "Tento účet je vázán na jiný PC."})
        ...
        async def send():
            try:
                ...
                if u: await u.send(embed=discord.Embed(title="🛡️ Ověření přihlášení", description=f"Potvrďte přístup tlačítkem níže.", color=0x38bdf8), view=AppAuthView(token, user.get("discord_id"), is_dm=True))
            except: pass
...
def api_get_profile_data(discord_id):
    ...
    try:
        ...
        if u_data:
            u = u_data[0]
            t_time = int(u.get("total_time") or 0); l_count = int(u.get("launch_count") or 0)
            ...
            if u.get("is_online"): app_status = "<span style='color: var(--success); font-weight:bold;'><i class='fas fa-circle'></i> Nyní hraje</span>"
            else: app_status = f"<span style='color: var(--text-muted);'><i class='fas fa-moon'></i> {u.get('last_active', 'Nikdy')}</span>"
        ...
        return _cors_jsonify({"joined_at": "Nenalezen", "status": "", "app_status": app_status, "stats": stats, "downloads": downloads, "sessions": sessions_data})
    except Exception as e: return _cors_jsonify({"error": str(e)}), 500
...
# ZDE JE TVŮJ NOVÝ SYSTÉM PRO STATUSY NA DISCORD
...
def update_statuses():
    ...
    s_db = request.form.get("status_db", "🟢 Databáze je stabilní a synchronizovaná.")
    s_global = request.form.get("status_global", "🟢 Všechny systémy softwaru jsou online a v provozu.")
    maint_dl = True if request.form.get("status_dl_manual") else False
    
    settings_to_update = {
        "status_db_msg": s_db,
        "status_global_msg": s_global,
        "status_dl_maintenance": str(maint_dl)
    }
        ...
        if request.form.get("send_to_discord"):
            async def send_status_embed():
                ...
                for guild in bot.guilds:
                    ...
                    if channel:
                        color = 0x10b981
                        dl_final_msg = "🟢 Stahování softwaru a instalátorů běží v pořádku."
                        if maint_dl:
                            color = 0xf59e0b
...
def dashboard_app_management():
    ...
    st_db = ""; st_global = ""; st_maint = False
    try:
        db = get_db()
        if db:
    ...
    # Napojení na nový status okénko z druhého souboru!
    return render_dashboard(HTML_APP_MANAGEMENT + HTML_STATUS_SECTION, 
                            soft_enabled=soft_enabled, dl_enabled=dl_enabled, 
                            status_db=st_db, status_global=st_global, status_dl_manual=st_maint,
...
class DynamicDownloadView(discord.ui.View):
    ...
    async def dl_btn(self, interaction, button):
        await interaction.response.send_message("Odesláno do DM.", ephemeral=True)
