...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=30) # Pamatuje si přihlášení na 30 dní
...
<div style="text-align: center; padding: 60px 20px; max-width: 800px; margin: 0 auto;">
    ...
    <p style="font-size: 1.2em; color: var(--text-main); line-height: 1.6; margin-bottom: 40px; background: rgba(30, 41, 59, 0.5); padding: 20px; border-radius: 10px; border-left: 4px solid var(--blue-main);">
        Projekt OIS IDPK je pokročilý fanouškovský software, pečlivě vytvořený podle reálné předlohy vnitřních informačních panelů používaných v autobusech Plzeňského kraje. Přinášíme autentický zážitek a moderní technologie přímo na vaši obrazovku.
    </p>
    ...
    <div style="display: flex; align-items: center; justify-content: center; gap: 30px; flex-wrap: wrap; background: var(--bg-panel); padding: 30px; border-radius: 15px; border: 1px solid #334155;">
        <img src="{{ logo_velke }}" alt="DataCoreBot Logo" style="max-width: 150px; height: auto; filter: drop-shadow(0px 10px 15px rgba(0,0,0,0.5));">
        <div style="text-align: left; max-width: 400px;">
            <h3 style="color: var(--warning); margin-top: 0; font-size: 1.4em; text-shadow: 0 0 5px rgba(245, 158, 11, 0.5);">Poháněno systémem DataCoreBot</h3>
            <p style="color: var(--text-muted); font-size: 0.95em; line-height: 1.5; margin: 0;">
...
HTML_STATS = """
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    <h2 style="margin: 0; color: var(--text-main);"><i class="fas fa-chart-bar" style="color:var(--blue-main);"></i> Statistiky Webu</h2>
</div>
...
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    <h3 style="color: var(--blue-main); margin-top: 0;">🌍 Návštěvnost podle zemí</h3>
    <table style="width: 100%;">
        <tr>
            <th>Země</th>
            <th>Počet zobrazení</th>
        </tr>
        {% for c_name, count in countries.items() %}
        <tr>
            <td style="font-weight: bold; color: var(--text-main);">{{ c_name }}</td>
            <td style="color: var(--blue-main); font-weight: bold;">{{ count }}</td>
        </tr>
        {% else %}
...
def check_session_validity():
    if request.path.startswith('/dashboard/') and request.path != '/dashboard/wait_auth' and request.path != '/dashboard/login_finalize':
...
@app.route('/')
def home(): 
    # Logování návštěvnosti do tabulky page_visits
    def log_visit(ip, country):
        try:
            db = get_db()
            if db:
                now_str = get_prague_time().strftime("%d.%m.%Y %H:%M")
                db.table("page_visits").insert({"ip": ip, "country": country, "visited_at": now_str}).execute()
        except: pass
...
def dashboard_stats():
    ...
    total_visits = 0; last_7_days = 0; countries = {}
    try:
        db = get_db()
        if db:
            ...
            for v in visits:
                ...
                c = v.get('country', 'Neznámá')
                countries[c] = countries.get(c, 0) + 1
    except Exception as e: flash(f"Chyba při načítání statistik: {e}", "error")
    return render_dashboard(HTML_STATS, total_visits=total_visits, last_7_days=last_7_days, countries=countries, deploy_time=DEPLOY_TIME)
