        ...
        if "drive.google.com" in file_url and "/d/" in file_url:
            ...
            if match:
                ...
                if 'text/html' in resp.headers.get('Content-Type', '').lower():
                    ...
                    token = None
                    for cookie in cj:
                        if cookie.name.startswith("download_warning"): token = cookie.value; break
                    if not token:
        ...
        def generate():
            try:
                while True:
                    ...
                    if not chunk: break
                    yield chunk
            except Exception as stream_err: send_log("⚠️ Spojení přerušeno", f"Uživateli `{nick}` se přerušilo stahování v půlce.\nDůvod: {stream_err}", 0xf59e0b)
            finally: resp.close()
        resp_headers = {'Content-Disposition': f'attachment; filename="OIS_IDPK_{version_name.replace(" ", "_")}.zip"', 'Content-Type': resp.headers.get('Content-Type', 'application/octet-stream')}
        if resp.headers.get('Content-Length'): resp_headers['Content-Length'] = resp.headers.get('Content-Length')
    ...
    def format_version(v):
        ...
        if eol_str:
            try:
                ...
                if (eol_dt - now).days <= 14: return f"~~• {v['version_name']}~~ ❌ *(Stahování ukončeno - blížící se konec podpory {eol_str})*"
...
async def send_user_dm(discord_identifier, title, description, color=0x38bdf8):
    ...
    try:
        for guild in bot.guilds:
            member = guild.get_member(int(discord_identifier)) if discord_identifier.isdigit() else discord.utils.find(lambda m: m.name.lower() == discord_identifier.lower() or (m.global_name and m.global_name.lower() == discord_identifier.lower()), guild.members)
            if member:
        ...
        for guild in bot.guilds:
            ...
            if member:
                ...
                for r_name in role_names_list:
                    ...
                    if role: await member.add_roles(role); assigned_any = True
                if assigned_any:
                    success = True
                    try: await member.send(embed=discord.Embed(title="🎉 Děkujeme za obrovskou podporu!", description=f"Byly ti přiděleny exkluzivní role:\n\n" + "\n".join([f"**{r}**" for r in role_names_list]), color=0x38bdf8))
                    except: pass
                break
    except: pass
    return success
...
async def announce_new_supporter(discord_nick, amount_str, message, role_names_list):
    ...
    for guild in bot.guilds:
        ...
        if channel:
            embed = discord.Embed(title="🎉 MÁME NOVÉHO PODPOROVATELE!", description=f"Uživatel **{discord_nick}** právě podpořil náš projekt a získal exkluzivní role na serveru i v aplikaci!", color=0xf59e0b)
...
def send_log(title, description, color=0x38bdf8):
    if bot.loop and bot.loop.is_running() and bot.is_ready(): asyncio.run_coroutine_threadsafe(async_send_log(title, description, color), bot.loop)

def _cors_jsonify(data): return jsonify(data)
        ...
        if discord_id:
            try:
                ...
                if db:
                    ...
                    if user and (not user[0].get("dashboard_access") or user[0].get("is_banned") or user[0].get("is_deleted")):
                        session.clear()
...
def check_version_access(db, app_version_from_pc, user):
    ...
    if not app_version_from_pc or str(app_version_from_pc).strip() == "": return {"allowed": False, "msg": "Nepodporovaná verze aplikace. Stáhněte si novou verzi přes náš Discord."}
    try:
    ...
    if bot.loop and bot.loop.is_running() and bot.is_ready():
        async def send_err():
            for guild in bot.guilds:
                ...
                if channel:
                    try: await channel.send(embed=discord.Embed(title=f"⚠️ APLIKAČNÍ CHYBA: {error_type}", description=f"**Hráč:** {nick} (`{discord_id}`)\n**Chyba:**\n`{msg}`", color=0xef4444, timestamp=get_prague_time()))
                    except: pass
                    break
...
def home(): 
    def log_visit(ip, cf_country):
        try:
            ...
            if any(r.get("visited_at", "").startswith(today_str) for r in existing): return
            ...
            db.table("page_visits").insert({"ip": clean_ip, "country": f"{country_code}|{country_name}|{region}", "visited_at": now_str}).execute()
        except: pass
    Thread(target=log_visit, args=(request.headers.get('X-Forwarded-For', request.remote_addr), request.headers.get('CF-IPCountry', 'Neznámá'))).start()
...
def supporters():
    try: s_data = process_supporters(get_db().table("supporters").select("*").eq("status", "completed").execute().data or []) if get_db() else []
    except: s_data = []
    return render_public(HTML_SUPPORTERS, supporters=s_data)
...
def public_stats():
    ...
    if search_query:
        ...
        if not searched_user: flash(f"Hráč '{search_query}' nebyl nalezen.", "warning")
    versions = db.table("software_versions").select("*").eq("is_active", True).order("id", desc=True).execute().data or []
    ...
    for s in sessions_today:
        try:
            st_str, et_str = s.get('start_time'), s.get('end_time')
            if st_str and et_str:
                st = datetime.strptime(st_str, "%d.%m.%Y %H:%M:%S" if st_str.count(':') == 2 else "%d.%m.%Y %H:%M")
                et = datetime.strptime(et_str, "%d.%m.%Y %H:%M:%S" if et_str.count(':') == 2 else "%d.%m.%Y %H:%M")
                diff = int((et - st).total_seconds() / 60)
                if diff > 0: today_mins += diff
        except: pass
    today_time_str = f"{today_mins // 60}h {today_mins % 60}m" if today_mins // 60 > 0 else f"{today_mins % 60}m"
    total_supporters = len(db.table("supporters").select("id").eq("status", "completed").execute().data or [])
    top_time_users = sorted([u for u in all_users if int(u.get('total_time') or 0) > 0], key=lambda x: int(x.get('total_time') or 0), reverse=True)[:3]
    top_launches_users = sorted([u for u in all_users if int(u.get('launch_count') or 0) > 0], key=lambda x: int(x.get('launch_count') or 0), reverse=True)[:3]
    try:
        ...
        top_lines, top_stops = all_lines[:10], all_stops[:10]
    except: top_lines, top_stops, all_lines, all_stops = [], [], [], []
    searched_user_lines, searched_user_stops = [], []
    if searched_user:
        try:
            searched_user_lines = db.table("user_stats_lines").select("*").eq("discord_id", searched_user.get('discord_id')).order("play_count", desc=True).limit(5).execute().data or []
            searched_user_stops = db.table("user_stats_stops").select("*").eq("discord_id", searched_user.get('discord_id')).order("announce_count", desc=True).limit(5).execute().data or []
        except: pass
    return render_public(HTML_PUBLIC_STATS, user_ver=user_ver, bt_ver=bt_ver, activated_users=activated_users, total_supporters=total_supporters, today_time_str=today_time_str, total_hours=total_hours, total_launches=total_launches, top_time=top_time_users, top_launches=top_launches_users, top_lines=top_lines, top_stops=top_stops, all_lines=all_lines, all_stops=all_stops, searched_user=searched_user, searched_user_lines=searched_user_lines, searched_user_stops=searched_user_stops)

@app.route('/api/supporters', methods=['GET', 'OPTIONS'])
def api_supporters():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    try: return _cors_jsonify({"supporters": process_supporters(get_db().table("supporters").select("name, amount, message, created_at").eq("status", "completed").execute().data or [])})
...
def bmac_webhook():
    ...
    try:
        ...
        if db:
            ...
            if discord_identifier and user_exists_sync(discord_identifier): status = 'completed'; sys_note = "Automaticky spárováno."
...
def secure_download(token):
    ...
    try:
        ...
        html = f"""<div style="background-color: var(--bg-panel); padding: 40px; border-radius: 10px; text-align: center; max-width: 600px; margin: 0 auto; border-top: 4px solid var(--success);"><h2 style="color: var(--success); margin-top: 0;"><i class="fas fa-check-circle"></i> Ověření úspěšné</h2><p style="color: var(--text-muted); font-size: 14px; margin-bottom: 30px;">Přihlášen jako: <strong>{user.get('nick', '')}</strong></p><div style="background-color: var(--bg-dark); padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #334155;"><h3 style="margin: 0 0 10px 0; color: var(--blue-main);">Projekt OIS IDPK</h3><p style="margin: 0; color: var(--text-main);">Instalátor: <strong>{v_data.get('version_name', '')}</strong></p></div><div id="download-area"><a href="#" onclick="startDownload()" class="btn btn-success" style="font-size: 18px; padding: 15px 30px; display: inline-block;" id="dl-btn"><i class="fas fa-download"></i> Stáhnout Soubor</a></div><div id="loading-area" style="display: none;"><div class="spinner" style="margin: 0 auto 10px auto; border-color: rgba(16, 185, 129, 0.3); border-top-color: #10b981;"></div><p style="color: var(--text-main); font-weight: bold;">Zahajuji stahování...</p><p style="color: var(--text-muted); font-size: 12px;">Generuji bezpečné připojení...</p></div><div id="success-area" style="display: none; margin-top: 20px;"><h3 style="color: var(--success); margin-top: 0;"><i class="fas fa-check"></i> Úspěšně zahájeno</h3><p style="color: var(--text-main); font-size: 14px; background: rgba(0,0,0,0.3); padding: 15px; border-radius: 8px; border-left: 3px solid var(--blue-main);">Stahování běží. Po stažení souboru jej nezapomeňte rozbalit pomocí programů jako <b>7-ZIP</b> nebo <b>WinRAR</b>.</p></div><div id="error-area" style="display: none; margin-top: 20px;"><h3 style="color: var(--danger); margin-top: 0;"><i class="fas fa-times-circle"></i> Stahování se nezdařilo</h3><p style="color: var(--text-main); font-size: 14px; background: rgba(239,68,68,0.1); padding: 15px; border-radius: 8px; border-left: 3px solid var(--danger);">Omlouváme se, stahování nebylo možné spustit.<br><b>Důvod:</b> <span id="error-msg"></span><br><br>Zkuste to prosím později, administrátor byl o chybě informován do logu.</p></div><style>.spinner {{ width: 40px; height: 40px; border: 4px solid; border-radius: 50%; animation: spin 1s linear infinite; }}@keyframes spin {{ 0% {{ transform: rotate(0deg); }} 100% {{ transform: rotate(360deg); }} }}</style><script>async function startDownload() {{document.getElementById('download-area').style.display = 'none';document.getElementById('loading-area').style.display = 'block';try {{let response = await fetch("/api/pre_download/{token}?v={version_id}");let data = await response.json();if (data.status === 'ok') {{window.location.href = "/api/stream_download/{token}?v={version_id}";setTimeout(() => {{document.getElementById('loading-area').style.display = 'none';document.getElementById('success-area').style.display = 'block';}}, 2000);}} else {{document.getElementById('loading-area').style.display = 'none';document.getElementById('error-area').style.display = 'block';document.getElementById('error-msg').innerText = data.message || "Neznámá chyba na serveru.";}}}} catch(e) {{document.getElementById('loading-area').style.display = 'none';document.getElementById('error-area').style.display = 'block';document.getElementById('error-msg').innerText = "Chyba připojení k serveru.";}}}}</script></div>"""
...
def api_pre_download(token):
    ...
    if last_log:
        try:
            ...
            last_dt = datetime.strptime(time_str, "%d.%m.%Y %H:%M:%S" if time_str.count(':') == 2 else "%d.%m.%Y %H:%M")
...
def api_status():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    try:
        if str(get_db().table("settings").select("setting_value").eq("setting_key", "software_enabled").execute().data[0].get('setting_value', 'True')).lower() == 'false':
...
def api_app_login():
    ...
    identifier, req_hwid, app_version = str(data.get("identifier", "")).strip(), str(data.get("hwid", "")), str(data.get("app_version", ""))
    db = get_db()
    try:
        if str(db.table("settings").select("setting_value").eq("setting_key", "software_enabled").execute().data[0].get('setting_value', 'True')).lower() == 'false': return _cors_jsonify({"status": "error", "message": "SOFTWARE JE NYNÍ VYPNUT."})
        client_ip = request.headers.get('X-Forwarded-For', request.remote_addr).split(',')[0].strip()
        user_resp = db.table("users").select("*").or_(f"discord_id.eq.{identifier},app_id.eq.{int(identifier)}" if identifier.isdigit() else f"nick.eq.{identifier}").execute()
        ...
        if not version_check["allowed"]:
            send_log("🛡️ Neoprávněný přístup verze", f"Uživatel `{user.get('nick')}` se pokusil zapnout zakázanou verzi: **{app_version}**", 0xf59e0b)
            return _cors_jsonify({"status": "error", "message": version_check["msg"]})
        db_hwid, db_ip = user.get("hwid"), user.get("ip_address")
        if not db_hwid or str(db_hwid) == "None" or str(db_hwid).strip() == "":
            if req_hwid and req_hwid.startswith("PC-"): db.table("users").update({"hwid": req_hwid, "ip_address": client_ip}).eq("discord_id", discord_id).execute()
        else:
            if str(db_hwid) != req_hwid:
                ...
                if not db_ip or str(db_ip).strip() == "": db.table("users").update({"ip_address": client_ip}).eq("discord_id", discord_id).execute()
...
def api_app_check():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    data = request.get_json(silent=True) or {}
    discord_id, req_hwid = str(data.get("discord_id", "")), str(data.get("hwid", ""))
    db = get_db()
    try:
...
def api_silent_check():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    data = request.get_json(silent=True) or {}
    discord_id, req_hwid, app_version = str(data.get("discord_id", "")), str(data.get("hwid", "")), str(data.get("app_version", ""))
    db = get_db()
    try:
        if str(db.table("settings").select("setting_value").eq("setting_key", "software_enabled").execute().data[0].get('setting_value', 'True')).lower() == 'false': return _cors_jsonify({"status": "error", "message": "SOFTWARE JE NYNÍ VYPNUT."})
        client_ip = request.headers.get('X-Forwarded-For', request.remote_addr).split(',')[0].strip()
        ...
        if str(db_hwid) != req_hwid:
            if db_ip and str(db_ip).strip() != "" and str(db_ip) == client_ip:
                ...
                return _cors_jsonify({"status": "success", "app_id": str(user.get("app_id", ""))})
            else: return _cors_jsonify({"status": "hwid_error", "message": "ZÁMEK HWID: Váš počítač ani IP adresa nesouhlasí s registrací."})
        if not db_ip or str(db_ip).strip() == "": db.table("users").update({"ip_address": client_ip}).eq("discord_id", discord_id).execute()
        return _cors_jsonify({"status": "success", "app_id": str(user.get("app_id", ""))})
    except Exception as e: return _cors_jsonify({"status": "error", "message": str(e)})
...
def api_app_ping():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    data = request.get_json(silent=True) or {}
    discord_id, action, session_id = str(data.get("discord_id", "")), data.get("action", "ping"), data.get("session_id", "")
    db = get_db()
...
def api_get_profile_data(discord_id):
    ...
    if not discord_id or discord_id == 'None' or discord_id.strip() == '': return _cors_jsonify({"error": "Chybí Discord ID"})
    try:
        db = get_db()
        ...
        if u_data:
            u = u_data[0]
            t_time = int(u.get("total_time") or 0)
            l_count = int(u.get("launch_count") or 0)
            stats = f"<div style='margin-bottom:5px;'><b style='color:var(--blue-main);'>{t_time // 60}h {t_time % 60}m</b> v aplikaci</div><div><b style='color:var(--blue-main);'>{l_count}x</b> spuštěno</div>"
            app_status = "<span style='color: var(--success); font-weight:bold;'><i class='fas fa-circle'></i> Nyní hraje</span>" if u.get("is_online") else f"<span style='color: var(--text-muted);'><i class='fas fa-moon'></i> {u.get('last_active', 'Nikdy')}</span>"
        joined_at = "Nenalezen na serveru"
        try:
            if bot.is_ready():
                for guild in bot.guilds:
                    member = guild.get_member(int(discord_id))
                    if member and member.joined_at: joined_at = member.joined_at.strftime("%d.%m.%Y"); break
        except: pass
...
def api_get_messages():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    data = request.get_json(silent=True) or {}
    discord_id, app_id = str(data.get("discord_id", "")), str(data.get("app_id", ""))
    db = get_db()
        ...
        for msg in all_msgs:
            ...
            if expires_at_str and expires_at_str.strip():
                try:
                    ...
                    if now > exp_dt: db.table("app_messages").update({"is_archived": True}).eq("message_id", msg["message_id"]).execute(); continue
                except: pass
            target_type, target_data = msg.get("target_type", "GLOBAL"), str(msg.get("target_data", ""))
            is_target = False
...
def api_mark_message_read():
    if request.method == 'OPTIONS': return _cors_jsonify({})
    data = request.get_json(silent=True) or {}
    discord_id, message_id = str(data.get("discord_id", "")), str(data.get("message_id", ""))
    db = get_db()
    ...
    try:
        ...
        if db:
            ...
            for v in visits:
                ...
                parts = c_raw.split('|')
                cc, c_name, reg = parts[0] if len(parts) > 0 else "", parts[1] if len(parts) > 1 else c_raw, parts[2] if len(parts) > 2 else ""
                ...
                try:
                    ...
                    if v_time.date() == now.date():
                        if v_time.strftime("%H:00") in chart_data_24h: chart_data_24h[v_time.strftime("%H:00")] += 1
                except: pass
    ...
    if db and discord_id:
        try:
            if action == 'save':
                ...
                updates = {"nick": nick, "role": r_str, "hwid": new_hwid, "dashboard_access": True if request.form.get("dashboard_access") else False}
                if not new_hwid: updates["ip_address"] = "" 
                db.table("users").update(updates).eq("discord_id", discord_id).execute()
...
def send_app_message():
    ...
    if db:
        try:
            ...
            db.table("app_messages").insert({"message_id": new_msg_id, "target_type": target_type, "target_data": target_data, "title": title, "content": content, "repeat": repeat, "link_url": link_url, "expires_at": expires_at, "is_archived": False, "created_at": now_str}).execute()
...
def edit_app_message():
    ...
    if db and msg_id:
        try:
            db.table("app_messages").update({"target_type": request.form.get("target_type", "GLOBAL"), "target_data": request.form.get("target_data", "").strip(), "title": request.form.get("title", "Zpráva od vývojáře"), "content": request.form.get("content", ""), "repeat": True if request.form.get("repeat") else False, "link_url": request.form.get("link_url", "") if request.form.get("has_link") else "", "expires_at": request.form.get("expires_at", "")}).eq("message_id", msg_id).execute()
...
def add_version():
    ...
    try:
        ...
        get_db().table("software_versions").insert({"version_name": v_name, "db_version": request.form.get("db_version"), "file_url": request.form.get("file_url"), "target_role": request.form.get("target_role"), "is_active": True, "eol_date": ""}).execute()
    ...
    except Exception as e: flash(f'Chyba při přidávání verze: {e}', 'error')
...
def add_team():
    ...
    if db:
        try:
            ...
            send_log("🤝 Nový člen týmu", f"Do týmu na webu byl přidán: **{request.form.get('name')}**.", 0x10b981)
        except: pass
...
def approve_claim():
    if not session.get('logged_in'): return redirect(url_for('dashboard_main'))
    claim_id, discord_nick, amount = request.form.get("claim_id"), request.form.get("discord_nick"), request.form.get("amount", "0")
    db = get_db()
...
def reject_claim():
    if not session.get('logged_in'): return redirect(url_for('dashboard_main'))
    claim_id, discord_nick, sys_note = request.form.get("claim_id"), request.form.get("discord_nick", ""), request.form.get("sys_note", "Zamítnuto administrátorem bez udání důvodu.")
    db = get_db()
    if db and claim_id:
        db.table("supporters").update({"status": "rejected", "sys_note": f"Zamítnuto: {sys_note} [{get_prague_time().strftime('%d.%m.%Y %H:%M')}]"}).eq("id", claim_id).execute()
...
def add_supporter():
    ...
    try: 
        db = get_db()
        d_nick, amt, msg = request.form.get("discord_nick", "").strip(), request.form.get("amount", "0 CZK"), request.form.get("message", "")
        db.table("supporters").insert({"name": request.form.get("name"), "discord_nick": d_nick, "amount": amt, "message": msg, "status": "completed", "sys_note": "Přidáno manuálně", "created_at": get_prague_time().strftime("%d.%m.%Y %H:%M")}).execute()
        if d_nick:
...
def update_statuses():
    ...
    dl_mode, dl_custom_icon, dl_custom_text = request.form.get("dl_status_mode", "auto"), request.form.get("dl_status_custom_icon", "🟢"), request.form.get("dl_status_custom_text", "")
    settings_to_update = {"dl_status_mode": dl_mode, "dl_status_custom_icon": dl_custom_icon, "dl_status_custom_text": dl_custom_text}
    try:
            ...
            async def send_status_embed():
                ...
                for guild in bot.guilds:
                    ...
                    if channel:
                        ...
                        soft_enabled, dl_enabled = True, True
                        for r in res:
                        ...
                        s_global = "🟢 Všechny systémy softwaru jsou online a v provozu." if soft_enabled else "🔴 Systém je nyní v režimu údržby (Software globálně vypnut)."
                        s_db = "🟢 Databáze je stabilní a synchronizovaná."
                        color = 0x10b981
                        if dl_mode == "auto":
                            if dl_enabled: s_dl = "🟢 Stahování softwaru a instalátorů běží v pořádku."
                            else: s_dl = "🔴 Stahování je momentálně administrátorem zakázáno."; color = 0xef4444
                        elif dl_mode == "maintenance": s_dl = "🟠 **Probíhá oprava systému.** Stahování některých souborů nemusí fungovat správně."; color = 0xf59e0b
                        else:
...
class PerDeleteConfirm(discord.ui.View):
    def __init__(self, target_id, author_id):
        ...
        self.target_id, self.author_id = target_id, author_id
...
async def keepalive_ping():
    try: await asyncio.to_thread(urllib.request.urlopen, urllib.request.Request("https://datacorebot.koyeb.app/api/keepalive", headers={'User-Agent': 'Mozilla/5.0'}), timeout=10)
    except: pass
        ...
        for guild in bot.guilds:
            ...
            if channel:
                ...
                if message.attachments:
                    embed.add_field(name="📎 Přílohy:", value="\n".join([f"• [{a.filename}]({a.url})" for a in message.attachments]), inline=False)
                    if message.attachments[0].url.lower().endswith(('png', 'jpg', 'jpeg', 'gif', 'webp')): embed.set_image(url=message.attachments[0].url)
                try: await channel.send(embed=embed)
                except: pass
                break
...
async def dm_view(ctx, discord_id: str):
    if not discord_id.isdigit(): return await ctx.send("❌ Zadej platné číselné ID uživatele.")
    ...
    try:
        ...
        await ctx.send(f"📄 Tady je posledních 100 zpráv ze soukromé konverzace s uživatelem `{user.display_name}`:", file=discord.File(io.BytesIO(log_content.encode('utf-8')), filename=f"ChatLog_{user.display_name}.txt"))
...
async def register(ctx, target_id: str = None):
    ...
    if target_id:
        if not (discord.utils.get(ctx.author.roles, name="web-sa") or discord.utils.get(ctx.author.roles, name="SM") or ctx.author.guild_permissions.administrator): return await ctx.send(f"❌ {ctx.author.mention} Nemáš oprávnění.")
        ...
        discord_id, nick, target_member = str(ctx.author.id), ctx.author.display_name, ctx.author
    now_str = get_prague_time().strftime("%d.%m.%Y %H:%M")
...
if __name__ == "__main__":
    ...
    else: print("KRITICKÁ CHYBA: DISCORD_TOKEN chybí! (Web běží dál bez Bota)")
    run_web()
