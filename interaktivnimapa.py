...
<div style="padding: 10px; max-width: 1600px; margin: auto;">
    ...
    <div id="map" style="width: 100%; height: 75vh; border-radius: 10px; border: 2px solid #334155; box-shadow: 0 4px 6px rgba(0,0,0,0.3); z-index: 1; position: relative;"></div>
    ...
    <style>
        .bus-marker { border-radius: 50%; border: 2px solid white; text-align: center; color: white; font-weight: bold; font-size: 10px; line-height: 20px; box-shadow: 0 0 5px rgba(0,0,0,0.5); position: relative; }
        ...
        .bg-orange { background-color: #f59e0b; color: #0f172a; }
        .bg-bug { background-color: #374151; border-color: #6b7280; border-style: dashed; color: #9ca3af; opacity: 0.65; }
                ...
                if(data.status === "success") {
                    ...
                    data.buses.forEach(bus => {
                        if(bus.lat && bus.lng) {
                            ...
                            let arrowHtml = '';
                            if(bus.bearing !== null && markerColor !== 'bg-gray' && markerColor !== 'bg-purple' && markerColor !== 'bg-bug') {
                                arrowHtml = `<div style="transform: rotate(${bus.bearing}deg); width: 100%; height: 100%; position: absolute; top:0; left:0;">
                                                <div style="width: 0; height: 0; border-left: 4px solid transparent; border-right: 4px solid transparent; border-bottom: 6px solid black; position: absolute; top: 2px; left: 50%; transform: translateX(-50%); z-index: 2;"></div>
                                             </div>`;
                            }
                            
                            let icon = L.divIcon({ className: shape + ' ' + markerColor, html: arrowHtml, iconSize: [24, 24] });
                            ...
                            if(IS_ADMIN) {
                                popupHTML += `
                                <div style="border-top: 1px solid #ef4444; margin-top:10px; padding-top:10px;">
...
def background_map_worker():
    global TRACKED_SPZS
    print("[MAPA] Inteligentní mozek (Striktní SPZ, Anti-Duplikátor, Směr) startuje...", flush=True)
...
def stranka_mapa_admin():
    ...
    admin_banner = '<div style="background:#dc2626; color:white; padding:10px; text-align:center; font-weight:bold; font-size:18px; z-index:9999; position:relative; margin-top:58px; box-shadow:0 4px 10px rgba(0,0,0,0.5);">ADMIN MODERATION ZAPNUTÉ</div>'
