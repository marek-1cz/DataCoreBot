...
.n-ad-back{background:#ef4444;color:#fff;}
.n-ad-back:hover{background:#dc2626;color:#fff;}
...
.bg-bug{background-color:#374151;}
...
async function fetchBuses(){
  try{
    ...
    data.buses.forEach(bus=>{
      ...
      // Design kolečka - číslo linky uvnitř!
      let displayLine = bus.line ? bus.line.replace(/\D/g, '') : '';
      if(displayLine.length > 3) displayLine = displayLine.slice(-3); // Zkrátíme na poslední 3 čísla (např 735)
      
      let textColor = (mc==='bg-orange' || mc==='bg-blue') ? '#0f172a' : '#ffffff';

      // Směrová šipka vně kolečka (vycentrovaná rotace kolem středu)
      let arrowHtml = '';
      if(bus.bearing !== null && mc !== 'bg-gray' && mc !== 'bg-purple' && mc !== 'bg-bug') {
          arrowHtml = `<div style="position:absolute; top:0; left:0; width:46px; height:46px; transform: rotate(${bus.bearing}deg); transform-origin: 23px 23px;">
                          <div style="width:0; height:0; border-left: 9px solid transparent; border-right: 9px solid transparent; border-bottom: 16px solid ${arrColor}; position:absolute; top: -7px; left: 14px; filter: drop-shadow(0px -1px 2px rgba(0,0,0,0.5));"></div>
                       </div>`;
      }

      let innerCircle = `<div class="${mc}" style="position:absolute; top:8px; left:8px; width:30px; height:30px; border-radius:50%; border:2px solid #fff; box-shadow:0 0 5px rgba(0,0,0,.5); display:flex; align-items:center; justify-content:center; font-size:10px; font-weight:900; color:${textColor}; z-index:5;">${displayLine}</div>`;
      
      let iconHtml = `<div style="position:relative; width:46px; height:46px;">${arrowHtml}${innerCircle}</div>`;
      
      let icon = L.divIcon({ className: '', html: iconHtml, iconSize: [46, 46], iconAnchor: [23, 23], popupAnchor: [0, -23] });
      ...
      if(IS_ADMIN) {
          ...
          <div style="border-top: 1px solid #ef4444; margin-top:10px; padding-top:10px; padding-left:13px; padding-right:13px; padding-bottom:13px;">
              ...
              <div style="display:flex; gap:5px; margin-top:5px;">
                  ...
                  <button onclick="event.stopPropagation(); adminSetSPZ('${bus.id}', event)" style="width:50%; background:#10b981; color:white; border:none; border-radius:4px; font-size:11px; cursor:pointer; font-weight:bold;">Uložit SPZ</button>
              </div>
              <div style="display:flex; gap:5px; margin-top:5px;">
                  <button onclick="event.stopPropagation(); adminRecheck('${bus.id}', event)" style="width:50%; background:#f59e0b; color:black; border:none; border-radius:4px; font-size:11px; cursor:pointer; font-weight:bold;">Hledat novou</button>
                  <button onclick="event.stopPropagation(); adminDelete('${bus.id}', event)" style="width:50%; background:#ef4444; color:white; border:none; border-radius:4px; font-size:11px; cursor:pointer; font-weight:bold;">Smazat tečku</button>
              </div>
              <div style="display:flex; gap:5px; margin-top:5px;">
                  ...
                  <button onclick="event.stopPropagation(); adminSetStatus('${bus.id}', event)" style="width:20%; background:#38bdf8; color:black; border:none; border-radius:4px; font-size:11px; cursor:pointer; font-weight:bold;">Uložit</button>
              </div>
          </div>`;
      }
      ...
      if(window.markersDict[bus.id]) {
          ...
          if(wasOpen) {
              // Zabrání přemazání textu při psaní
              let isFocused = false;
