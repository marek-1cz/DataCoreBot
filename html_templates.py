            ...
            <form action="/dashboard/edit_user" method="POST" style="border-top: 1px solid #334155; padding-top: 15px; margin-top: 15px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        ...
                        <input type="text" name="hwid" id="modalHwid" placeholder="Pro odblokování smažte text zde (vymaže se HWID i IP)">
...
<div style="text-align: center; padding: 60px 20px; max-width: 800px; margin: 0 auto;">
    ...
    <div style="font-size: 1.1em; color: var(--text-main); line-height: 1.6; margin-bottom: 40px; background: rgba(30, 41, 59, 0.5); padding: 25px; border-radius: 10px; border-left: 4px solid var(--blue-main); text-align: left;">
        ...
        <p style="margin-bottom:0;">Celý projekt vznikl z nadšení pro dopravu, technologie a informační systems ve veřejné dopravě. Projekt není oficiálním produktem ani službou dopravců nebo organizací veřejné dopravy a nijak s nimi nespolupracuje.</p>
    </div>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 40px;">
    ...
    <div style="background: var(--bg-panel); padding: 25px; border-radius: 10px; border-left: 5px solid #a855f7; box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
        <div style="color: var(--text-muted); font-size: 14px; text-transform: uppercase; font-weight: bold;">Aktuální verze (BT)</div>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 40px; margin-bottom: 40px;">
    <div style="background: var(--bg-panel); padding: 30px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        ...
        <div style="text-align: center; margin-top: 20px;"><button class="btn btn-dark" onclick="document.getElementById('all-lines-modal').style.display='flex'" style="font-size: 13px; width: 100%;"><i class="fas fa-list"></i> Zobrazit kompletní databázi linek</button></div>
    </div>
    ...
    <div style="background: var(--bg-panel); padding: 30px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        ...
        <div style="text-align: center; margin-top: 20px;"><button class="btn btn-dark" onclick="document.getElementById('all-stops-modal').style.display='flex'" style="font-size: 13px; width: 100%;"><i class="fas fa-list"></i> Zobrazit kompletní databázi zastávek</button></div>
    </div>
</div>
...
<div class="modal-overlay" id="all-lines-modal">
    <div class="modal" style="width: 700px;">
        <div style="width: 100%;">
            <h2 style="color: var(--blue-main); margin-top: 0; border-bottom: 1px solid #334155; padding-bottom: 10px; display: flex; justify-content: space-between;">
                <span><i class="fas fa-route"></i> Databáze odehraných linek</span><span onclick="document.getElementById('all-lines-modal').style.display='none'" style="cursor:pointer; color:var(--danger);"><i class="fas fa-times"></i></span>
            </h2>
            ...
            <div style="max-height: 500px; overflow-y: auto;">
                <table style="width: 100%;" id="lines-table">
                    <tr><th>Linka</th><th style="text-align:right;">Počet odehrání</th></tr>
                    {% for l in all_lines %}
                    <tr class="line-row"><td style="color: white; font-weight:bold;">{{ l.get('line_name', '') }}</td><td style="text-align:right; color: var(--blue-main); font-weight:bold;">{{ l.get('play_count', 0) }}x</td></tr>
                    {% endfor %}
                </table>
            </div>
...
<div class="modal-overlay" id="all-stops-modal">
    <div class="modal" style="width: 700px;">
        <div style="width: 100%;">
            <h2 style="color: var(--success); margin-top: 0; border-bottom: 1px solid #334155; padding-bottom: 10px; display: flex; justify-content: space-between;">
                <span><i class="fas fa-map-marker-alt"></i> Databáze vyhlášených zastávek</span><span onclick="document.getElementById('all-stops-modal').style.display='none'" style="cursor:pointer; color:var(--danger);"><i class="fas fa-times"></i></span>
            </h2>
            ...
            <div style="max-height: 500px; overflow-y: auto;">
                <table style="width: 100%;" id="stops-table">
                    <tr><th>Zastávka</th><th style="text-align:right;">Počet vyhlášení</th></tr>
                    {% for s in all_stops %}
                    <tr class="stop-row"><td style="color: white; font-weight:bold;">{{ s.get('stop_name', '') }}</td><td style="text-align:right; color: var(--success); font-weight:bold;">{{ s.get('announce_count', 0) }}x</td></tr>
                    {% endfor %}
                </table>
            </div>
...
<script>
    function filterLines() { let input = document.getElementById("lines-search").value.toUpperCase(); document.querySelectorAll(".line-row").forEach(r => r.style.display = r.innerText.toUpperCase().indexOf(input) > -1 ? "" : "none"); }
    function filterStops() { let input = document.getElementById("stops-search").value.toUpperCase(); document.querySelectorAll(".stop-row").forEach(r => r.style.display = r.innerText.toUpperCase().indexOf(input) > -1 ? "" : "none"); }
    function filterPersonalLines() { let input = document.getElementById("personal-lines-search").value.toUpperCase(); document.querySelectorAll(".pline-row").forEach(r => r.style.display = r.innerText.toUpperCase().indexOf(input) > -1 ? "" : "none"); }
    function filterPersonalStops() { let input = document.getElementById("personal-stops-search").value.toUpperCase(); document.querySelectorAll(".pstop-row").forEach(r => r.style.display = r.innerText.toUpperCase().indexOf(input) > -1 ? "" : "none"); }
</script>
"""
...
<div style="max-width: 500px; margin: 50px auto; background-color: var(--bg-panel); padding: 40px; border-radius: 10px; border-top: 4px solid var(--blue-main); box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
    ...
    <form method="POST">
        <label style="color: var(--text-muted); font-size: 12px; font-weight: bold;">JMÉNO ZADANÉ NA BUY ME A COFFEE</label><input type="text" name="bmac_name" placeholder="Např. Jan Novák" required style="margin-bottom: 20px;">
        <label style="color: var(--text-muted); font-size: 12px; font-weight: bold; display: block;">VÁŠ DISCORD NICK</label><input type="text" name="discord_nick" placeholder="Např. marekk_czz" required>
...
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
    ...
    <div style="display: flex; gap: 15px; flex-wrap: wrap;">
        {% for cc, data in country_totals.items() %}<div style="background: rgba(0,0,0,0.3); border: 1px solid #334155; padding: 10px 20px; border-radius: 8px; display: flex; align-items: center; gap: 10px;"><img src="{{ data.flag }}" alt="" style="border-radius: 3px; box-shadow: 0 0 5px rgba(0,0,0,0.5);"><span style="color: var(--text-main); font-weight: bold;">{{ data.name }}</span><span style="background: var(--blue-main); color: #000; padding: 2px 8px; border-radius: 12px; font-weight: 900; font-size: 12px;">{{ data.count }}</span></div>
        {% else %}<div style="color: var(--text-muted);">Zatím žádná data k zobrazení.</div>{% endfor %}
    </div>
</div>
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    ...
    <table style="width: 100%;"><tr><th>Stát / Region</th><th>Počet zobrazení</th></tr>
        {% for c_name, data in region_totals.items() %}<tr><td style="font-weight: bold; color: var(--text-main); display: flex; align-items: center; gap: 10px;">{% if data.flag %}<img src="{{ data.flag }}" alt="" style="border-radius: 3px; box-shadow: 0 0 5px rgba(0,0,0,0.5);">{% endif %}{{ c_name }}</td><td style="color: var(--blue-main); font-weight: bold; font-size: 16px;">{{ data.count }}</td></tr>
        {% else %}<tr><td colspan="2" style="text-align: center; color: var(--text-muted);">Zatím žádná data k zobrazení. Tabulka "page_visits" je prázdná.</td></tr>{% endfor %}
    </table>
</div>
<script>
    const labels7d = {{ labels_7d | safe }}, data7d = {{ data_7d | safe }}, labels24h = {{ labels_24h | safe }}, data24h = {{ data_24h | safe }};
...
<div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 20px;">
    <div class="card" style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; flex: 1; min-width: 250px; text-align: center; border: 1px solid #334155;">
        <div style="display: flex; justify-content: center; margin-bottom: 20px;">
            {% if soft_enabled %}<div style="width: 80px; height: 80px; border-radius: 50%; background-color: #10b981; box-shadow: 0 0 25px #10b981; display: flex; align-items: center; justify-content: center; transition: all 0.3s;"><i class="fas fa-globe" style="color: white; font-size: 35px;"></i></div>
            {% else %}<div style="width: 80px; height: 80px; border-radius: 50%; background-color: #ef4444; box-shadow: 0 0 25px #ef4444; display: flex; align-items: center; justify-content: center; transition: all 0.3s;"><i class="fas fa-power-off" style="color: white; font-size: 35px;"></i></div>{% endif %}
        </div>
        ...
        <form action="/dashboard/toggle_software" method="POST">
            ...
            {% if soft_enabled %}<button type="submit" class="btn" style="background-color: #ef4444; color: white; border: none; padding: 12px 25px; border-radius: 50px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%;"><i class="fas fa-times-circle"></i> Vypnout software</button>
            {% else %}<button type="submit" class="btn" style="background-color: #10b981; color: white; border: none; padding: 12px 25px; border-radius: 50px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%;"><i class="fas fa-check-circle"></i> Zapnout software</button>{% endif %}
        </form>
    </div>
    <div class="card" style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; flex: 1; min-width: 250px; text-align: center; border: 1px solid #334155;">
        <div style="display: flex; justify-content: center; margin-bottom: 20px;">
            {% if dl_enabled %}<div style="width: 80px; height: 80px; border-radius: 50%; background-color: #3b82f6; box-shadow: 0 0 25px #3b82f6; display: flex; align-items: center; justify-content: center; transition: all 0.3s;"><i class="fas fa-download" style="color: white; font-size: 35px;"></i></div>
            {% else %}<div style="width: 80px; height: 80px; border-radius: 50%; background-color: #ef4444; box-shadow: 0 0 25px #ef4444; display: flex; align-items: center; justify-content: center; transition: all 0.3s;"><i class="fas fa-times" style="color: white; font-size: 35px;"></i></div>{% endif %}
        </div>
        ...
        <form action="/dashboard/toggle_downloads" method="POST">
            <input type="hidden" name="return_to" value="app_management"><input type="hidden" name="new_status" value="{% if dl_enabled %}False{% else %}True{% endif %}">
            {% if dl_enabled %}<button type="submit" class="btn" style="background-color: #ef4444; color: white; border: none; padding: 12px 25px; border-radius: 50px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%;"><i class="fas fa-times-circle"></i> Zakázat stahování</button>
            {% else %}<button type="submit" class="btn" style="background-color: #10b981; color: white; border: none; padding: 12px 25px; border-radius: 50px; font-size: 16px; font-weight: bold; cursor: pointer; width: 100%;"><i class="fas fa-check-circle"></i> Povolit stahování</button>{% endif %}
        </form>
    </div>
</div>
"""
...
HTML_NOTIFICATIONS = """
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2 style="margin: 0; color: var(--text-main);"><i class="fas fa-bell" style="color:#f59e0b;"></i> Systém Oznámení</h2></div>
<div style="display: flex; gap: 20px; flex-wrap: wrap;">
    <div style="flex: 1; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px; border-top: 4px solid var(--warning);">
        ...
        <form action="/dashboard/send_app_message" method="POST">
            ...
            <label style="color: var(--text-muted); font-size: 13px;">Cílení:</label><select name="target_type" id="target_type" onchange="toggleTargetData()" style="margin-bottom: 10px;"><option value="GLOBAL">Všichni uživatelé</option><option value="ROLE">Podle Rolí</option><option value="USERS">Vybraní uživatelé</option></select>
            <div id="target_data_container" style="display: none;"><label style="color: var(--text-muted); font-size: 13px;" id="target_label">Specifikace:</label><input type="text" name="target_data" id="target_data" placeholder=""></div>
            <div style="background-color: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; margin: 15px 0;"><label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-main); font-size: 13px;"><input type="checkbox" name="has_link" id="has_link" onchange="toggleLinkUrl()" style="width: auto; margin: 0;"> Přidat tlačítko s odkazem</label></div>
            <div id="link_url_container" style="display: none;"><label style="color: var(--text-muted); font-size: 13px;">URL adresa tlačítka:</label><input type="url" name="link_url" placeholder="https://..."></div>
            ...
            <label style="color: var(--text-muted); font-size: 13px;">Expirace:</label><input type="text" name="expires_at" placeholder="Např. 31.12.2026 23:59 (Volitelné)">
    ...
    <div style="flex: 2; min-width: 300px;">
        <div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <div style="overflow-x: auto;">
                <table><tr><th>Nadpis</th><th>Cílení</th><th>Opakování</th><th>Expirace</th><th>Akce</th></tr>
                    {% for m in messages %}{% if not m.get('is_archived') %}
                    <tr>
                        <td style="color: var(--text-main); font-weight: bold;">{{ m.get('title', '') }}</td>
                        <td>{% if m.get('target_type') == 'GLOBAL' %}<span class="role-tag" style="background-color: #3b82f6; color: white;">Globálně</span>{% elif m.get('target_type') == 'ROLE' %}<span class="role-tag" style="background-color: #a855f7; color: white;">Role: {{ m.get('target_data', '') }}</span>{% else %}<span class="role-tag" style="background-color: #ef4444; color: white;">Hráči: {{ m.get('target_data', '')[:15] }}...</span>{% endif %}</td>
                        <td>{% if m.get('repeat') %}<span style="color:var(--warning); font-size: 12px; font-weight:bold;">Ano</span>{% else %}<span style="color:var(--success); font-size: 12px; font-weight:bold;">Jen jednou</span>{% endif %}</td>
                        <td style="color: var(--text-muted); font-size: 12px;">{{ m.get('expires_at', 'Nikdy') or 'Nikdy' }}</td>
                        <td style="display: flex; gap: 5px;">
                            <button type="button" class="btn btn-warning" style="padding: 5px 10px; font-size: 12px;" data-id="{{ m.get('message_id', '') }}" data-title="{{ (m.get('title') or '') | e }}" data-content="{{ (m.get('content') or '') | e }}" data-type="{{ (m.get('target_type') or '') | e }}" data-data="{{ (m.get('target_data') or '') | e }}" data-url="{{ (m.get('link_url') or '') | e }}" data-exp="{{ (m.get('expires_at') or '') | e }}" data-repeat="{{ 'true' if m.get('repeat') else 'false' }}" onclick="openEditMessageModal(this)"><i class="fas fa-edit"></i></button>
                            <form action="/dashboard/archive_app_message" method="POST" style="margin:0;"><input type="hidden" name="message_id" value="{{ m.get('message_id', '') }}"><button type="submit" class="btn btn-dark" style="padding: 5px 10px; font-size: 12px;"><i class="fas fa-archive"></i></button></form>
                            <form action="/dashboard/delete_app_message" method="POST" style="margin:0;"><input type="hidden" name="message_id" value="{{ m.get('message_id', '') }}"><button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;" onclick="return confirm('Opravdu smazat?')"><i class="fas fa-trash"></i></button></form>
                        </td>
                    </tr>
                    {% endif %}{% else %}<tr><td colspan="5" style="text-align: center; color: var(--text-muted);">Žádná aktivní oznámení.</td></tr>{% endfor %}
                </table>
            </div>
        </div>
        <div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; opacity: 0.8;">
            ...
            <div style="overflow-x: auto;">
                <table><tr><th>Nadpis</th><th>Cílení</th><th>Vytvořeno</th><th>Akce</th></tr>
                    {% for m in messages %}{% if m.get('is_archived') %}<tr><td style="color: var(--text-muted);">{{ m.get('title', '') }}</td><td style="font-size: 12px; color: var(--text-muted);">{{ m.get('target_type', '') }}</td><td style="color: var(--text-muted); font-size: 12px;">{{ m.get('created_at', '') }}</td><td><form action="/dashboard/delete_app_message" method="POST" style="margin:0;"><input type="hidden" name="message_id" value="{{ m.get('message_id', '') }}"><button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;"><i class="fas fa-trash"></i></button></form></td></tr>{% endif %}{% endfor %}
                </table>
            </div>
        </div>
    </div>
...
<div class="modal-overlay" id="editMessageModal"><div class="modal" style="width: 500px; border-top: 5px solid var(--warning);"><div style="width: 100%;"><h2 style="color: var(--warning); margin-top: 0; border-bottom: 1px solid #334155; padding-bottom: 10px;"><i class="fas fa-edit"></i> Úprava Oznámení</h2><form action="/dashboard/edit_app_message" method="POST"><input type="hidden" name="message_id" id="em_id"><label style="color: var(--text-muted); font-size: 13px;">Nadpis:</label><input type="text" name="title" id="em_title" required><label style="color: var(--text-muted); font-size: 13px;">Text:</label><textarea name="content" id="em_content" rows="4" required></textarea><label style="color: var(--text-muted); font-size: 13px;">Typ cílení:</label><select name="target_type" id="em_type" required><option value="GLOBAL">Globálně</option><option value="ROLE">Podle Rolí</option><option value="USERS">Vybraní uživatelé</option></select><label style="color: var(--text-muted); font-size: 13px;">Data cílení:</label><input type="text" name="target_data" id="em_data"><label style="color: var(--text-muted); font-size: 13px;">Odkaz tlačítka:</label><input type="url" name="link_url" id="em_url"><label style="color: var(--text-muted); font-size: 13px;">Expirace:</label><input type="text" name="expires_at" id="em_exp"><div style="background-color: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; margin: 15px 0;"><label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-main); font-size: 13px;"><input type="checkbox" name="repeat" id="em_repeat" style="width: auto; margin: 0;"> Zobrazovat uživatelům DOKOLA</label></div><button type="submit" class="btn btn-warning" style="width: 100%; margin-top: 15px;"><i class="fas fa-save"></i> Uložit změny</button></form><button type="button" class="btn" style="width: 100%; margin-top: 10px; background: transparent; border: 1px solid #334155; color: var(--text-muted);" onclick="document.getElementById('editMessageModal').style.display='none'">Zrušit</button></div></div></div>
<script>
    function toggleTargetData() { let t=document.getElementById('target_type').value,c=document.getElementById('target_data_container'),i=document.getElementById('target_data'); if(t==='GLOBAL'){c.style.display='none';i.removeAttribute('required');}else{c.style.display='block';i.setAttribute('required','true');} }
    function toggleLinkUrl() { document.getElementById('link_url_container').style.display=document.getElementById('has_link').checked?'block':'none'; }
    function openEditMessageModal(b) { document.getElementById('em_id').value=b.getAttribute('data-id')||""; document.getElementById('em_title').value=b.getAttribute('data-title')||""; document.getElementById('em_content').value=b.getAttribute('data-content')||""; document.getElementById('em_type').value=b.getAttribute('data-type')||"GLOBAL"; document.getElementById('em_data').value=b.getAttribute('data-data')||""; document.getElementById('em_url').value=b.getAttribute('data-url')||""; document.getElementById('em_exp').value=b.getAttribute('data-exp')||""; document.getElementById('em_repeat').checked=b.getAttribute('data-repeat')==='true'; document.getElementById('target_type').value=b.getAttribute('data-type')||"GLOBAL"; toggleTargetData(); document.getElementById('editMessageModal').style.display='flex'; }
</script>
"""
...
<div style="display: flex; gap: 20px; flex-wrap: wrap;">
    <div style="flex: 1; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
        ...
        <form action="/dashboard/add_version" method="POST">
            <input type="text" name="version_name" placeholder="Zobrazený Název" required>
            <input type="text" name="db_version" placeholder="Verze Databáze" required>
            <input type="text" name="file_url" placeholder="Odkaz na stažení" required>
            <select name="target_role" required><option value="User">User</option><option value="BT">BETA TESTER</option><option value="DEV_SA">DEV / SERVER ADMIN</option></select>
...
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; margin-top: 20px;">
    <h3 style="color: var(--blue-main); margin-top: 0;">📦 Tabulka vydaných verzí</h3>
    <div style="overflow-x: auto;">
        <table><tr><th>Název</th><th>Verze DB</th><th>Stav</th><th>Role</th><th>Akce</th></tr>
            {% for v in versions %}{% set is_active = (v.get('is_active', True) | string | lower) != 'false' %}
            <tr style="opacity: {{ '1' if is_active else '0.5' }};">
                <td><strong>{{ v.get('version_name', '') }}</strong></td><td style="color: var(--warning); font-family: monospace;">{{ v.get('db_version', '') }}</td>
                <td>{% if is_active %}<span class="role-tag" style="background-color: var(--success); color: white;">Aktivní</span>{% else %}<span class="role-tag" style="background-color: var(--danger); color: white;">Zablokováno</span>{% endif %}<br><span style="font-size: 11px; color: var(--text-muted);">Konec: {{ v.get('eol_date', '') or 'Nikdy' }}</span></td>
                <td>{{ v.get('target_role') }}</td>
                <td>
                    <button type="button" class="btn btn-warning" style="padding: 5px 10px; font-size: 12px;" data-id="{{ v.get('id', '') }}" data-name="{{ (v.get('version_name') or '') | e }}" data-db="{{ (v.get('db_version') or '') | e }}" data-url="{{ (v.get('file_url') or '') | e }}" data-role="{{ (v.get('target_role') or '') | e }}" data-active="{{ 'true' if is_active else 'false' }}" data-eol="{{ (v.get('eol_date') or '') | e }}" onclick="openEditVerModal(this)"><i class="fas fa-edit"></i> Úprava</button>
                    <form action="/dashboard/delete_version" method="POST" style="display:inline;"><input type="hidden" name="version_id" value="{{ v.get('id', '') }}"><button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;" onclick="return confirm('Odebrat?')"><i class="fas fa-trash"></i> Smazat</button></form>
                </td>
            </tr>
            {% else %}<tr><td colspan="5" style="text-align: center; color: var(--text-muted);">Zatím nebyly přidány žádné verze.</td></tr>{% endfor %}
        </table>
    </div>
</div>

<div class="modal-overlay" id="editVerModal">
    <div class="modal"><div style="width: 100%;">
        <h2 style="color: var(--warning); margin-top: 0; border-bottom: 1px solid #334155; padding-bottom: 10px;"><i class="fas fa-edit"></i> Řízení Verze</h2>
        <form action="/dashboard/edit_version" method="POST">
            ...
            <select name="target_role" id="ev_role" required><option value="User">User</option><option value="BT">BETA TESTER</option><option value="DEV_SA">DEV / SERVER ADMIN</option></select>
            <div style="background-color: rgba(0,0,0,0.3); padding: 10px; border-radius: 5px; margin: 15px 0;"><label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-main); font-size: 13px;"><input type="checkbox" name="is_active" id="ev_active" style="width: auto; margin: 0;"><b>Tato verze je aktivní</b></label></div>
            <input type="text" name="eol_date" id="ev_eol" placeholder="Konec podpory (Např. 31.12.2026)">
...
<script>
    function openEditVerModal(btn) { document.getElementById('ev_id').value = btn.getAttribute('data-id')||""; document.getElementById('ev_name').value = btn.getAttribute('data-name')||""; document.getElementById('ev_db').value = btn.getAttribute('data-db')||""; document.getElementById('ev_url').value = btn.getAttribute('data-url')||""; document.getElementById('ev_role').value = btn.getAttribute('data-role')||"User"; document.getElementById('ev_active').checked = btn.getAttribute('data-active')==='true'; document.getElementById('ev_eol').value = btn.getAttribute('data-eol')||""; document.getElementById('editVerModal').style.display='flex'; }
</script>
"""
...
<div style="display: flex; gap: 20px; flex-wrap: wrap;">
    <div style="flex: 1; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
        ...
        <form action="/dashboard/add_pending_role" method="POST">
            <input type="text" name="discord_identifier" placeholder="Discord Nick" required>
            ...
            <button type="submit" class="btn" style="width: 100%; margin-top: 15px;">Rezervovat</button>
        </form>
    </div>
    <div style="flex: 2; min-width: 300px; background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
        ...
        <div style="overflow-x: auto;">
            <table><tr><th>Discord Identifikátor</th><th>Role</th><th>Akce</th></tr>
                {% for p in pending %}<tr><td><strong>{{ p.get('discord_identifier', '') }}</strong></td><td>{{ p.get('roles', '') }}</td><td><form action="/dashboard/delete_pending_role" method="POST" style="display:inline;"><input type="hidden" name="pending_id" value="{{ p.get('id', '') }}"><button type="submit" class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;"><i class="fas fa-trash"></i></button></form></td></tr>{% else %}<tr><td colspan="3" style="text-align: center; color: var(--text-muted);">Zatím žádné rezervace.</td></tr>{% endfor %}
            </table>
        </div>
    </div>
</div>
...
HTML_TEAM_ADD = """
<h2 style="color: var(--blue-main);">Správa Týmu</h2>
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    <form action="/dashboard/add_team" method="POST">
        <input type="text" name="name" placeholder="Jméno" required>
        <input type="text" name="discord_nick" placeholder="Nick" required>
        ...
        <button type="submit" class="btn" style="width: 100%;">Přidat člena</button>
    </form>
</div>
"""

HTML_IDS = """
<h2 style="color: var(--blue-main);">Správa ID</h2>
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    <table>
        <tr><th>ID</th><th>Nick</th></tr>
        {% for user in users %}<tr><td>#{{ user.get('app_id') }}</td><td>{{ user.get('nick') }}</td></tr>{% endfor %}
    </table>
</div>
"""

HTML_DASHBOARD_MAIN = """
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2 style="margin: 0; color: var(--text-main);">{{ title }}</h2><div id="refresh-timer" style="color: var(--text-muted); font-size: 13px; background: rgba(0,0,0,0.3); padding: 8px 12px; border-radius: 6px; border: 1px solid #334155; font-weight: bold;"><i class="fas fa-sync-alt" style="color: var(--blue-main);"></i> Aktualizace za: <span id="timer-sec" style="color: white;">60</span>s</div></div>
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px;">
    <div style="overflow-x: auto;">
        <table id="usersTable">
            ...
            {% for user in users %}
            <tr>
                <td style="font-weight: bold; color: var(--blue-main);">#{{ user.get('app_id', '') }}</td><td><strong>{{ user.get('nick', '') }}</strong></td>
                ...
                <td data-sort="{{ role_weight }}">{{ user.get('role', '') }} {% if user.get('dashboard_access') %}<i class="fas fa-shield-alt" style="color:var(--blue-main); font-size:12px; margin-left:5px;"></i>{% endif %}</td>
                ...
                <td><button class="btn btn-dark" style="padding: 5px 10px; font-size: 12px;" data-app-id="{{ user.get('app_id', '') }}" data-discord-id="{{ user.get('discord_id', '') }}" data-nick="{{ (user.get('nick') or '') | e }}" data-roles="{{ (user.get('role') or '') | e }}" data-hwid="{{ (user.get('hwid') or '') | e }}" data-banned="{{ user.get('is_banned', False) }}" data-deleted="{{ user.get('is_deleted', False) }}" data-db-access="{{ user.get('dashboard_access', False) }}" data-reg-at="{{ (user.get('registered_at') or '') | e }}" onclick="openModal(this)"><i class="fas fa-edit"></i> Upravit</button></td>
            </tr>
            {% else %}<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Žádní uživatelé nenalezeni.</td></tr>{% endfor %}
            </tbody>
        </table>
    </div>
</div>
<script>
    let timeLeft = 60; setInterval(() => { if(timeLeft > 0) { timeLeft--; let secEl = document.getElementById('timer-sec'); if(secEl) secEl.innerText = timeLeft; } if(timeLeft === 0) { timeLeft = -1; location.reload(); } }, 1000);
    let sortDir = {};
    function sortTable(n) { let table, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0; table = document.getElementById("usersTable"); switching = true; dir = sortDir[n] === "asc" ? "desc" : "asc"; sortDir[n] = dir; while (switching) { switching = false; rows = table.rows; for (i = 1; i < (rows.length - 1); i++) { shouldSwitch = false; x = rows[i].getElementsByTagName("TD")[n]; y = rows[i + 1].getElementsByTagName("TD")[n]; let xContent = x.hasAttribute("data-sort") ? x.getAttribute("data-sort") : x.innerHTML.replace(/<[^>]*>?/gm, '').trim(); let yContent = y.hasAttribute("data-sort") ? y.getAttribute("data-sort") : y.innerHTML.replace(/<[^>]*>?/gm, '').trim(); if (!isNaN(xContent) && !isNaN(yContent)) { xContent = parseFloat(xContent); yContent = parseFloat(yContent); } else { xContent = xContent.toLowerCase(); yContent = yContent.toLowerCase(); } if (dir == "asc") { if (xContent > yContent) { shouldSwitch = true; break; } } else if (dir == "desc") { if (xContent < yContent) { shouldSwitch = true; break; } } } if (shouldSwitch) { rows[i].parentNode.insertBefore(rows[i + 1], rows[i]); switching = true; switchcount ++; } } }
</script>
"""
