...
<div id="activeActions" style="margin-top: 20px;"><div style="display: flex; gap: 10px;"><button type="submit" name="action" value="save" class="btn" style="flex: 2;"><i class="fas fa-save"></i> Uložit</button><button type="submit" name="action" value="ban" id="btnBan" class="btn btn-warning" style="flex: 1;"><i class="fas fa-ban"></i> Dát BAN</button><button type="submit" name="action" value="unban" id="btnUnban" class="btn btn-success" style="flex: 1; display: none;"><i class="fas fa-check"></i> Un-BAN</button></div><div style="margin-top: 10px;"><button type="submit" name="action" value="delete" class="btn btn-danger" style="width: 100%;" onclick="return confirm('Smazat účet? (Zablokuje ID, umožní novou registraci)')"><i class="fas fa-trash"></i> Smazat (Soft Delete)</button></div></div>
<div id="deletedActions" style="display: none; margin-top: 20px;"><p style="color: var(--danger); font-weight: bold; text-align: center; margin-top: 0; margin-bottom: 5px;">Tento účet je smazaný.</p><div style="display: flex; gap: 10px;"><button type="submit" name="action" value="restore" class="btn btn-success" style="flex: 1;"><i class="fas fa-undo"></i> Obnovit účet</button><button type="submit" name="action" value="hard_delete" class="btn btn-dark" style="flex: 1;" onclick="return confirm('PERMANENTNÍ SMAZÁNÍ: Tato akce kompletně vymaže veškerá data. Pokračovat?')"><i class="fas fa-skull"></i> Smazat permanentně</button></div></div>
</div></div>
</form>
...
<div style="background-color: var(--bg-panel); padding: 20px; border-radius: 10px; border-top: 4px solid var(--warning); margin-bottom: 20px;">
    ...
    <div style="overflow-x: auto;">
        <table>
            ...
            {% else %}
            <tr><td colspan="4" style="text-align: center; color: var(--text-muted);">Zatím žádné žádosti o HWID a IP reset.</td></tr>
            {% endfor %}
        </table>
    </div>
</div>
    ...
    <div style="overflow-x: auto;">
        <table>
            ...
            <tr style="opacity: 0.7;">
                ...
                <td>{{ 'HWID a IP Reset' if f.get('type') == 'HWID' or f.get('type') == 'HWID_IP' else ('Admin Bypass' if f.get('type') == 'ADMIN_BYPASS' else 'Zpětná vazba') }}</td>
...
<script>
    function rejectHwid(btn) {
        ...
        let reason = prompt("Zadejte důvod zamítnutí pro hráče " + nick + ":", "Reset HWID a IP nyní není možný.");
        if (reason) {
...
<div style="padding: 20px;">
    ...
    <div id="map" style="width: 100%; height: 75vh; border-radius: 10px; border: 2px solid #334155; box-shadow: 0 4px 6px rgba(0,0,0,0.3);"></div>
    ...
    <div id="timetable-modal" class="modal">
      ...
      <div class="modal-content" style="background: white; border-radius: 8px; padding: 20px; max-width: 600px;">
        <div id="timetable-content" style="color: black;">Načítám jízdní řád...</div>
      </div>
    ...
    <style>
        /* Tvorba kulatých barevných teček na mapě (místo standardních špendlíků) */
        .bus-marker {
            border-radius: 50%;
        ...
        .bus-green { background-color: #10b981; }
        .bus-red { background-color: #ef4444; }
        .bus-blue { background-color: #3b82f6; }
    </style>
    ...
    <script>
        ...
        // Funkce na zobrazení jízdního řádu (využije náš nový Python endpoint)
        async function showTimetable(busId) {
            ...
            content.innerHTML = "<p>Načítám jízdní řád z Inflow...</p>";
            ...
            try {
                ...
                content.innerHTML = "<p style='color:red;'>Chyba při načítání.</p>";
            }
        }
                ...
                if(data.status === "success") {
                    ...
                    data.buses.forEach(bus => {
                        if(bus.lat && bus.lng) {
                            ...
                            // Logika barev podle tvého popisu:
                            // Záporné obří číslo = modrá, do 240 s (4 min) = zelená, 240+ = červená
                            let markerColor = "bus-green";
                            let delayText = Math.round(bus.delay / 60) + " min";
                            
                            if (bus.delay < 0) {
                                markerColor = "bus-blue";
                                delayText = "Před odjezdem";
                            ...
                            let myIcon = L.divIcon({
                                className: 'bus-marker ' + markerColor,
                                iconSize: [24, 24],
                                html: ''
                            });
                            ...
                            // Bublina (Popup) - teď tam máme i SPZ a tlačítko na Jízdní řád!
                            let popupHTML = `
                                <div style="font-family:sans-serif; font-size: 14px;">
                                    <b>Linka:</b> ${bus.line}<br>
                                    <b>Cíl:</b> ${bus.destination || "Neznámý"}<br>
                                    <b>Zpoždění:</b> ${delayText}<br>
                                    <b>SPZ:</b> <span style="color:#f59e0b;font-weight:bold;">${bus.spz}</span><br>
        ...
        fetchBuses();
        setInterval(fetchBuses, 10000); // Aktualizace každých 10 sekund
    </script>
</div>
"""
